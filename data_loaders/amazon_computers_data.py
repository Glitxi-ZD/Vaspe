"""
Amazon Computers数据集加载模块 - VPN友好版
支持通过网络下载（使用系统代理/VPN）
"""
import torch
import numpy as np
...
class AmazonComputersDataset:
    ...
    def _load_data(self, data_path):
        ...
        try:
            ...
            print(f"  Successfully downloaded/loaded Amazon Computers")
            return
        except Exception as e:
            error_msg = f"""
            ERROR: Could not load Amazon Computers dataset!
    ...
    def _load_from_local_cache(self, data_path):
        """从本地缓存加载"""
        possible_paths = [
            os.path.join(data_path, 'Computers'),
            os.path.join(data_path, 'Amazon', 'Computers'),
            data_path,
            './data/Computers',
    ...
    def _process_loaded_data(self, data, num_classes=None):
        ...
        # Amazon Computers .pt 文件格式可能是 (dict, None, Data) 元组
    ...
    def _setup_link_prediction(self, train_ratio, val_ratio):
        ...
        print(f"Amazon Computers - Link Prediction Split:")
    ...
    def _setup_node_classification(self, train_ratio, val_ratio):
        ...
        print(f"Amazon Computers - Node Classification Split:")
...
def create_amazon_computers_dataset(root='./data', task='link_prediction',
                                    train_ratio=0.8, val_ratio=0.1, seed=42, **kwargs):
    """创建Amazon Computers数据集"""
    possible_roots = [
        root,
        os.path.join(root, 'Computers'),
        os.path.join(root, 'Amazon', 'Computers'),
        './data/Computers',
        './data/Amazon/Computers',
        './data',
    ]
...
# 向后兼容
class AmazonComputersLinkPredictionDataset(AmazonComputersDataset):
...
class AmazonComputersNodeClassificationDataset(AmazonComputersDataset):
