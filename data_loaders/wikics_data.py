...
class WikiCSDataset:
    ...
    def _load_data(self, data_path):
        ...
        """
        print(f"Loading WikiCS dataset...")
        ...
        try:
            ...
            print(f"  Successfully downloaded/loaded WikiCS")
            return
        except Exception as e:
            error_msg = f"""
            ERROR: Could not load WikiCS dataset!
            ...
            To fix:
            - Ensure VPN is connected if behind firewall
            - Or manually download from: https://github.com/pmernyei/wiki-cs-dataset
            """
    ...
    def _load_from_local_cache(self, data_path):
        """从本地缓存加载"""
        possible_paths = [
            os.path.join(data_path, 'WikiCS'),
            os.path.join(data_path, 'wiki_cs'),
            data_path,
            './data/WikiCS',
            './data/wiki_cs',
        ]
    ...
    def _process_loaded_data(self, data):
        ...
        # WikiCS .pt 文件格式可能是 (dict, None, Data) 元组
    ...
    def _setup_link_prediction(self, train_ratio, val_ratio):
        ...
        edges = []
        seen = set()
        edge_index_np = self.full_edge_index.numpy()
        ...
        print(f"WikiCS - Link Prediction Split:")
    ...
    def _setup_node_classification(self, train_ratio, val_ratio):
        ...
        print(f"WikiCS - Node Classification Split:")
...
def create_wikics_dataset(root='./data', task='link_prediction',
                          train_ratio=0.8, val_ratio=0.1, seed=42, **kwargs):
    """创建WikiCS数据集"""
    possible_roots = [
        root,
        os.path.join(root, 'WikiCS'),
        './data/WikiCS',
        './data',
    ]
...
# 向后兼容
class WikiCSLinkPredictionDataset(WikiCSDataset):
...
class WikiCSNodeClassificationDataset(WikiCSDataset):
