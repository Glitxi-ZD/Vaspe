...
class CiteseerLinkPredictionDataset:
    """
    CiteSeer数据集 - 支持链接预测和节点分类双任务
    ...
    def __init__(self, data_path='./data', task='link_prediction', 
        ...
        # 加载原始数据
        dataset = Planetoid(root=data_path, name='CiteSeer')
        data = dataset[0]
...
def create_citeseer_dataset(root='./data', task='link_prediction', 
    ...
    Returns:
        CiteseerLinkPredictionDataset 实例
    """
    return CiteseerLinkPredictionDataset(
        data_path=root,
        task=task,
...
class CiteseerDataset:
    ...
    def __init__(self, data_path='./data'):
        dataset = create_citeseer_dataset(root=data_path)
        self.data = dataset
...
class CiteseerNodeClassificationDataset(CiteseerLinkPredictionDataset):
    """CiteSeer节点分类数据集（向后兼容）"""
